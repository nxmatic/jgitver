...
public interface GitVersionCalculator extends AutoCloseable, MetadataProvider {
    ...
    static GitVersionCalculator location(File gitRepositoryLocation) {
        ...
        for (ClassLoader classLoader : classLoaders) {
            class ClassLoaderInfoTracer extends Throwable {

                private static final long serialVersionUID = 1L;

                public ClassLoaderInfoTracer(ClassLoader loader) {
                    super("Loading GitVersionCalculatorBuilder");
                    this.loader = loader;
                }
                ...
                final ClassLoader interfaceClassLoader = GitVersionCalculatorBuilder.class.getClassLoader();

                @Override
                public String getMessage() {
                    return String.format("[thread=%s,thread class loader=%s,class loader=%s,interface class loader=%s] %s",
                            thread.toString(), threadContextLoader.toString(), loader.toString(),
                            interfaceClassLoader.toString(), super.getMessage());
                }

                Throwable caught;

                void addSelfAsSuppressed(Throwable thrown) {
                    thrown.addSuppressed(this);
                    caught = thrown;
                }
            ...
            final ClassLoaderInfoTracer trace = new ClassLoaderInfoTracer(classLoader);

            Iterator<GitVersionCalculatorBuilder> builders = ServiceLoader.load(GitVersionCalculatorBuilder.class,
                    classLoader).iterator();
            try {
                if (hasNextBuilder(builders, classLoader)) {
                    GitVersionCalculator gvc;
                    try {
                        gvc = builders.next().build(gitRepositoryLocation);
                        return gvc;
                    } catch (IOException ex) {
                        throw new IllegalStateException("cannot open git repository under: " + gitRepositoryLocation,
                                ex);
                    }
                }
            } catch (Throwable error) {
                trace.addSelfAsSuppressed(error);
            }
        }

        try {
            Class builderClass = Class.forName("fr.brouillard.oss.jgitver.impl.GitVersionCalculatorImplBuilder");
            GitVersionCalculatorBuilder builder = (GitVersionCalculatorBuilder) builderClass.getConstructor()
                                                                                            .newInstance();
    ...
    static boolean hasNextBuilder(Iterator<GitVersionCalculatorBuilder> it, ClassLoader classLoader) {
        try {
            return it.hasNext();
        } catch (Throwable ex) {
            System.err.println("hasNextBuilder: Cannot load GitVersionCalculatorBuilder from " + classLoader + ": "
                    + ex.getMessage());
            return false; // ignore
        }
    }
    ...
    /**
     * Return the version to use for the current git repository depending on the HEAD position. Can return cached values
     * if no parameters have changed since last computation.
     *
    ...
    /**
     * Return the version to use for the current git repository depending on the HEAD position. If the computation was
     * already performed, ust return the last computed value.
     *
     ...
     *
     * @param forceComputation true to force computation even if no computation parameters have changed, false to allow
     *            to retrieve the value from the last computed version.
     * @return a string representation of this version.
     */
     ...
     * Example: "master, integration" This method overrides the usage of {@link #setQualifierBranchingPolicies(List)}
    ...
    /**
     * Sets as an array the policies that will be applied to try to build a qualifier from the branch of the HEAD. This
     * method overrides the usage of {@link #setNonQualifierBranches(String)} &
     * {@link #setQualifierBranchingPolicies(List)}.
     *
    ...
    /**
     * Sets as a list the policies that will be applied to try to build a qualifier from the branch of the HEAD. This
     * method overrides the usage of {@link #setNonQualifierBranches(String)} &amp;
     * {@link #setQualifierBranchingPolicies(BranchingPolicy...)}.
     *
     * @param policies an array of policies to apply can be empty
     ...
     * 
     * @param useLongFormat if true and useGitCommitId, then commitId will be prepended with a 'g' to be compliant with
     *            `git describe --long` format
     * @return itself to chain settings
     */
     ...
     *
     * @param useDefaultBranchingPolicy if true, appends {@link BranchingPolicy#DEFAULT_FALLBACK} as last branching
     *            policy
     * @return itself to chain settings
     */
    ...
    /**
     * Defines a regexp search pattern that will match tags identifying a version. The provided regexp MUST contains at
     * least one selector group that will represent the version extracted from the tag.
     * 
     * @param pattern a non null string representing a java regexp pattern able to match tag containing versions
     * @return itself to chain settings
     * @throws java.util.regex.PatternSyntaxException if the given string cannot be parsed as a correct
