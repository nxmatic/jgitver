...
public final class JGitverUtils {
  ...
  public static final Properties PLUGIN_PROPERTIES = pluginProperties();
  public static final String PLUGIN_GAV =
      String.format(
          "%s:%s:%s",
          PLUGIN_PROPERTIES.get("groupId"),
          PLUGIN_PROPERTIES.get("artifactId"),
          PLUGIN_PROPERTIES.get("version"));

  public static final String PLUGIN_POM_PROPERTIES_LOCATION =
      "META-INF/maven/"
  ...
  public static final BiFunction<String, String, Boolean> IS_JGITVER_PLUGIN_ARTIFACT =
      (groupId, artifactId) ->
          JGitverUtils.EXTENSION_GROUP_ID.equalsIgnoreCase(groupId)
              && JGitverUtils.EXTENSION_ARTIFACT_ID.equalsIgnoreCase(artifactId);

  public static final Predicate<Plugin> IS_JGITVER_PLUGIN =
      plugin -> IS_JGITVER_PLUGIN_ARTIFACT.apply(plugin.getGroupId(), plugin.getArtifactId());

  public static final Predicate<Dependency> IS_JGITVER_DEPENDENCY =
      dependency ->
          IS_JGITVER_PLUGIN_ARTIFACT.apply(dependency.getGroupId(), dependency.getArtifactId());
  ...
  /**
   * Returns the version of the Maven plugin.
   *
   * <p>This method reads the 'version' property from the 'pom.properties' file that is loaded from
   * the classpath. The 'pom.properties' file is generated by Maven during the build and contains
   * the GroupId, ArtifactId, and Version (GAV) of the plugin.
   *
   * @return the version of the Maven plugin.
   */
  public static String pluginVersion() {
    return PLUGIN_PROPERTIES.getProperty("version");
  }

  /**
   * Checks if the current context is running as an extension.
   *
   * <p>This method checks the current thread's context class loader. If it's an instance of
   * ClassRealm, it then checks if the realm's ID contains the plugin's GroupId:ArtifactId:Version
   * (GAV). This is a way to determine if the current context is running as an extension.
   *
  ...
  /**
   * Checks if the current context is running as a plugin.
   *
   * <p>This method checks the current thread's context class loader. If it's an instance of
   * ClassRealm, it then checks if the realm's ID contains the plugin's GroupId:ArtifactId:Version
   * (GAV). This is a way to determine if the current context is running as a plugin.
   *
   * @return true if the current context is running as a plugin, false otherwise.
   */
  public static boolean isRunningAsAPlugin() {
  ...
  // Loads and returns the Maven plugin properties from the 'pom.properties' file located in the
  // classpath at '/META-INF/maven/{groupId}/{artifactId}/pom.properties'.
  // Returns the loaded properties, or default properties if the file could not be read.
  private static Properties pluginProperties() {
    try (InputStream inputStream =
        JGitverUtils.class.getClassLoader().getResourceAsStream(PLUGIN_POM_PROPERTIES_LOCATION)) {
      if (inputStream == null) {
        throw new FileNotFoundException(
            String.format(
                "%s : %s",
                PLUGIN_POM_PROPERTIES_LOCATION, "JGitverUtils class loader can't find the file"));
      }
      Properties properties = new Properties();
      properties.load(inputStream);
      return properties;
    } catch (IOException ignored) {
      return mockPluginProperties();
    }
  }

  private static Properties mockPluginProperties() {
    Properties properties = new Properties();
    properties.put("artifactId", EXTENSION_ARTIFACT_ID);
    properties.put("groupId", EXTENSION_GROUP_ID);
    properties.put("version", "0-SNAPSHOT");
    return properties;
  }
}
